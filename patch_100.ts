...
const safeFileContent = fileContent.replace(/\\\\/g, '\\\\\\\\').replace(/\`/g, '\\\\`').replace(/\\$/g, '\\\\$');

const replacement = \`      } else if (lowerInput.includes('100 ข้อ') || lowerInput.includes('สมบูรณ์ที่สุด') || lowerInput.includes('100') && lowerInput.includes('โปรแกรม')) {
      ...
      } else if ((lowerInput.includes('ai offline')\`;
